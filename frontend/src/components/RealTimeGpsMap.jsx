...
function riskGlow(score) {
  ...
  if (score >= 20) return "rgba(34,211,238,0.7)"
...
export default function RealTimeGpsMap({ points = [], onSelectPoint = null, refreshInterval = 8000 }) {
    ...
    import("leaflet").then((L) => {
      ...
      mapPoints.forEach((pt) => {
        ...
        const icon = L.divIcon({
          ...
          html: `<div style="position:relative;width:${size*4}px;height:${size*4}px;display:flex;align-items:center;justify-content:center;">
        ...
        const popupHtml = `<div style="background:#0f172a;border:1px solid ${color};border-radius:10px;padding:14px 16px;font-family:monospace;color:#f8fafc;min-width:220px;box-shadow:0 0 24px ${glow};">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            ...
            <span style="font-size:11px;font-weight:800;padding:2px 7px;border-radius:4px;background:${color}22;color:${color};border:1px solid ${color}55;">${classLabel(pt.risk_score)} ${pt.risk_score}/100</span>
          </div>
          <div style="font-size:15px;font-weight:900;color:${color};font-family:monospace;margin-bottom:4px;">${pt.ip}</div>
          <div style="font-size:12px;color:#cbd5e1;margin-bottom:10px;">?? ${pt.city ? pt.city + ", " : ""}${pt.country}</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;">
            ...
            <div><span style="color:#64748b;">VPN/Tor:</span><br><b style="color:${pt.vpn_indicator ? "#f59e0b" : "#22c55e"}">${pt.vpn_indicator ? "? Proxy/Tor" : "? Clean"}</b></div>
            <div><span style="color:#64748b;">Confidence:</span><br><b>${pt.confidence || 75}%</b></div>
          </div>
        </div>`
        marker.bindPopup(popupHtml, { maxWidth: 280, className: "gps-dark-popup", closeButton: true })
        marker.on("click", () => { setSelected(pt); if (onSelectPoint) onSelectPoint(pt) })
        marker.addTo(map)
  ...
  return (
    <div style={{ position:"relative", borderRadius:"14px", overflow:"hidden", border:"1px solid rgba(56,189,248,0.2)", background:"#060d1a" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 18px", background:"linear-gradient(90deg,rgba(6,13,26,.95),rgba(15,23,42,.95))", borderBottom:"1px solid rgba(56,189,248,0.15)", backdropFilter:"blur(10px)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          {highCount > 0 && <span style={{ fontSize:"11px", fontWeight:800, padding:"3px 10px", borderRadius:"6px", background:"rgba(245,158,11,0.15)", color:"#fbbf24", border:"1px solid rgba(245,158,11,0.3)" }}>?? {highCount} HIGH</span>}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", minHeight:"480px" }}>
        <div style={{ position:"relative" }}>
          ...
          <canvas ref={canvasRef} style={{ position:"absolute", top:0, left:0, pointerEvents:"none", zIndex:999 }} />
          <div style={{ position:"absolute", bottom:"10px", left:"10px", zIndex:1000, background:"rgba(6,13,26,0.85)", backdropFilter:"blur(8px)", border:"1px solid rgba(56,189,248,0.2)", borderRadius:"6px", padding:"4px 10px", fontSize:"10px", color:"#475569", fontFamily:"monospace" }}>
            ...
            <>
              ...
              <div style={{ padding:"14px 16px", flex:1, display:"flex", flexDirection:"column", gap:"10px", fontSize:"11px" }}>
                {[
                  ...
                  ["VPN / Tor Signal", selected.vpn_indicator ? "?? Proxy/Tor Detected" : "? Direct / Clean"],
