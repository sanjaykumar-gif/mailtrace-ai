...
0 -14 Td
(Version 1.0.0 | SIH Production Edition) Tj
...
function makeStreamObj(num, text) {
  const len = Buffer.byteLength(text, 'latin1')
  return `${num} 0 obj << /Length ${len} >> stream\n${text}\nendstream\nendobj`
}

const font1 = '9 0 obj << /Type /Font /Subtype /Type1 /BaseFont /Helvetica >> endobj'
const font2 = '10 0 obj << /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >> endobj'
...
const objStrings = [
  ...
  makeStreamObj(6, content_p1),
  makeStreamObj(7, content_p2),
  makeStreamObj(8, content_p3),
  font1,
  font2
]
...
const xrefPos = Buffer.byteLength(output, 'latin1')
output += `xref\n0 ${objStrings.length + 1}\n0000000000 65535 f \n`
for (const offset of offsets) {
  output += `${offset.toString().padStart(10, '0')} 00000 n \n`
}

output += `trailer << /Size ${objStrings.length + 1} /Root 1 0 R >>\nstartxref\n${xrefPos}\n%%EOF\n`

const targetPath = path.join(__dirname, 'MailTrace_AI_Target_Users_and_Complete_Dashboard_Guide.pdf')
fs.writeFileSync(targetPath, Buffer.from(output, 'latin1'))
